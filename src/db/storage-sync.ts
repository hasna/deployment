...
export const DEPLOYMENT_STORAGE_TABLES = STORAGE_TABLES;

type StorageTable = (typeof STORAGE_TABLES)[number];
type Row = Record<string, unknown>;
export type StorageMode = "local" | "hybrid" | "remote";

export const DEPLOYMENT_STORAGE_ENV = "HASNA_DEPLOYMENT_DATABASE_URL";
export const DEPLOYMENT_STORAGE_FALLBACK_ENV = "DEPLOYMENT_DATABASE_URL";
export const DEPLOYMENT_STORAGE_MODE_ENV = "HASNA_DEPLOYMENT_STORAGE_MODE";
export const DEPLOYMENT_STORAGE_MODE_FALLBACK_ENV = "DEPLOYMENT_STORAGE_MODE";
export const STORAGE_DATABASE_ENV = [DEPLOYMENT_STORAGE_ENV, DEPLOYMENT_STORAGE_FALLBACK_ENV] as const;
export const STORAGE_MODE_ENV = [DEPLOYMENT_STORAGE_MODE_ENV, DEPLOYMENT_STORAGE_MODE_FALLBACK_ENV] as const;
...
function normalizeStorageMode(value: string): StorageMode {
  ...
  throw new Error(`Unknown deployment storage mode: ${value}`);
}
...
export async function getStoragePg(): Promise<PgAdapterAsync> {
  ...
  if (!url) throw new Error("Missing HASNA_DEPLOYMENT_DATABASE_URL or DEPLOYMENT_DATABASE_URL");
...
export async function storagePush(options?: { tables?: string[] }): Promise<SyncResult[]> {
  ...
  try {
    ...
    for (const table of parseStorageTables(options?.tables)) results.push(await pushTable(db, remote, table));
...
export async function storagePull(options?: { tables?: string[] }): Promise<SyncResult[]> {
  ...
  try {
    await runStorageMigrations(remote);
    const results: SyncResult[] = [];
    for (const table of parseStorageTables(options?.tables)) results.push(await pullTable(remote, db, table));
...
export function getSyncMetaAll(): SyncMeta[] {
  ...
  return db.query<SyncMeta, []>("SELECT table_name, last_synced_at, direction FROM _deployment_sync_meta ORDER BY table_name, direction").all();
}
...
export function parseStorageTables(tables?: string[]): StorageTable[] {
  ...
  if (invalid.length > 0) throw new Error(`Unknown deployment sync table(s): ${invalid.join(", ")}`);
...
function recordSyncMeta(db: Database, direction: "push" | "pull", results: SyncResult[]): void {
  ...
  const statement = db.prepare(
    "INSERT INTO _deployment_sync_meta (table_name, last_synced_at, direction) VALUES (?, ?, ?) ON CONFLICT(table_name, direction) DO UPDATE SET last_synced_at = excluded.last_synced_at",
  );
...
function ensureSyncMetaTable(db: Database): void {
  db.exec("CREATE TABLE IF NOT EXISTS _deployment_sync_meta (table_name TEXT NOT NULL, last_synced_at TEXT, direction TEXT NOT NULL CHECK(direction IN ('push', 'pull')), PRIMARY KEY (table_name, direction))");
}
